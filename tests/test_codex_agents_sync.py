...
class GenerateCodexAgentsSyncTest(unittest.TestCase):
    """generate_codex_agents: ai/codex/_AGENTS.md が prompt_base.md + nyaruko.md +
    ...
    元となる3ファイルの単純連結（関数の実装と同じ順序: prompt_base + 空行 +
    character + 空行 + codex_base）を Python 側で再現し、コミット済み
    _AGENTS.md と突き合わせる。
    """
    ...
    def test_committed_agents_md_matches_concatenation_of_sources(self):
        ...
        character = (REPO_ROOT / "ai/common/characters/nyaruko.md").read_text(encoding="utf-8")
        ...
        self.assertEqual(
            ...
            "after editing prompt_base.md / characters/nyaruko.md / codex_base.md",
        )
