...
The pr-review shared body lives in `ai/common/pr_review_core.md` and is loaded at runtime by Claude (`` !`/bin/cat ~/.claude/common/pr_review_core.md` `` in the command) and Gemini (`!{cat ~/.gemini/common/pr_review_core.md}`). For Codex, the same scripts generate `ai/codex/skills/pr-review/SKILL.md` from `skill_head.md` + `pr_review_core.md` + `skill_tail.md`; edit those sources, not the generated `SKILL.md`. The pr-comment-review shared body follows the same pattern: `ai/common/pr_comment_review_core.md` is loaded at runtime by Claude/Gemini, and the same scripts generate `ai/codex/skills/pr-comment-review/SKILL.md` from `skill_head.md` + `pr_comment_review_core.md`. The pr-comment-implement shared body follows it too: `ai/common/pr_comment_implement_core.md` is loaded at runtime by Claude/Gemini, and the same scripts generate `ai/codex/skills/pr-comment-implement/SKILL.md` from `skill_head.md` + `pr_comment_implement_core.md`.
