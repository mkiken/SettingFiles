...
| write-tests | `write_tests_core.md` | Gemini adapter is a generated skill (not a command); Claude adapter keeps `model`/`argument-hint`/`$ARGUMENTS` handling in the frontmatter/tail around the runtime include; adapter-head/tail bits: confirmation primitive (Codex's multi-select fallback is a plain-text numbered list — `request_user_input` has no multi-select) |
