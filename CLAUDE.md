...
`herdr tab create` / `herdr pane split` have no option to launch a command at creation time (`--cwd`/`--label`/`--env`/`--focus` only) — unlike tmux's `new-window "cmd"`, injecting into a freshly created pane is only possible via `herdr pane run <pane_id> <command>` (sends text + Enter, no built-in wait). Running `pane run` immediately after `tab create`/`pane split` races the new pane's shell startup and can silently drop the injected text before the shell is ready to receive input — confirmed as the root cause of `gm-pr-review`/`cx-pr-review` failing to launch in new Herdr tabs. A freshly created plain shell pane stays at `agent_status=unknown` (no AI detected yet), so `herdr wait agent-status --status idle` never fires and cannot be used to detect shell readiness. The reliable check, verified live: `pane run` an echoed marker string, then `herdr wait output <pane_id> --match <marker> --source recent --timeout <ms>` to confirm it reached the screen before sending the real command (see `_herdr_wait_shell_ready` in `shell/zsh/alias/ai/ai.zsh`).
...
**Herdr integrations**: `mac/scripts/herdr.sh` runs the official Claude and Codex installers in isolated staging, validates their registrations against the managed configuration, then transactionally deploys the hook scripts. Never run `herdr integration install` against live Claude or Codex configuration, especially the managed `~/.codex/hooks.json`; always use the repository helper. Gemini has no installer integration and relies only on Herdr's screen-manifest detection. This is the herdr→AI direction (detection/state reporting); the reverse AI→herdr direction (an agent driving the `herdr` CLI) is a separate concern handled by the vendored `ai/common/skills/herdr/SKILL.md` (see Standalone skills above). `sync_herdr_skill` (`mac/scripts/common.sh`), called once from `mac/updates/claude.sh` before `setup_ai_skills`, pulls the upstream `SKILL.md` on every `mac/update` and overwrites the vendored copy on a diff — but never stages it, so a human always reviews the change via `git diff` before committing. The `notify-rich` plugin (`terminal/herdr/plugins/notify-rich/notify-on-agent-status.sh`) also renames the tab (`herdr tab rename`) to prefix it with an AI-identifier + status icon on every `pane.agent_status_changed` event, mirroring tmux's window-icon scheme: the status comes from `herdr tab get`'s tab-aggregated `agent_status` (already merged across panes by Herdr), the identifier from the firing pane's `agent`, and the pre-existing label is recovered via `tmux_emoji.py`'s `strip_emoji_prefix` before re-prefixing (idle/unknown strips the icon back to the bare label). If the stripped label is purely numeric OR one of Herdr's known agent-detection auto-labels (e.g. `"1"` or `"Claude Code"` — Herdr renames a detected AI tab's label to the agent's display name, not just a number, so the numeric-only check alone stopped catching this case), it substitutes the pane's conversation title (truncated to 20 chars) instead, so untitled tabs read as their conversation summary rather than an index or a generic agent name; a label the user renamed to something else is left untouched. This detection is centralized in `shell/tmux/tmux_window_name.py`'s `is_herdr_default_label` (CLI: `is-herdr-default-label <base-label>`, exit 0/1), reused by the plugin so the known-label set has one home. Herdr runs `[[events]]` plugin hooks with the same stripped `PATH` as `[[keys.command]]` popups — no Homebrew dirs, `python3` is the system 3.9, `jq` is macOS's `/usr/bin/jq`, and `herdr` itself resolves only via the injected `HERDR_BIN_PATH` — so Homebrew-only tools like `terminal-notifier` silently vanish (tab renames kept working while every Mac notification failed for a day); the plugin therefore appends the Homebrew bin dirs to `PATH` (append, not prepend, so test `fake_bin` stubs keep precedence). When debugging a plugin, check `herdr plugin log list --plugin <id>` first — it records each invocation's stderr and exit code; reproducing in your own full-`PATH` shell proves nothing about the hook environment.
