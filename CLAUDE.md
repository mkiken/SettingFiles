...
When a `[[keys.command]]` popup needs to act on the pane that triggered it (not the popup itself), read `HERDR_ACTIVE_PANE_ID` / `HERDR_ACTIVE_PANE_CWD` — `HERDR_PANE_ID` is unset inside a popup and only identifies the popup pane itself. Confirmed by injecting a probe binding and inspecting `env | grep HERDR` from inside a live popup. `herdr pane send-text <pane_id> <text>` takes exactly two positional arguments and does not treat `--` as an end-of-options separator — passing one leaks a literal `--` into the inserted text; pass the two arguments directly.
