...
Find concrete runtime failures in changed code: wrong control flow, null/nil/undefined dereference, races, off-by-one, API misuse, resource leaks, unsafe casts, infinite loops, or missing termination. Do not report style, formatting, lint-only, security, or test-only issues.

Use parent-provided metadata, full diff, line-numbered diff, existing comments NDJSON, and local-mode flag. In local mode use `read_file`, `glob`, or `grep_search`; otherwise use `gh api` via `run_shell_command`. Do not refetch existing comments.
