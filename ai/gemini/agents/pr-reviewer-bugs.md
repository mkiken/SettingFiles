...
The parent provides metadata, full diff, line-numbered diff, existing comments NDJSON, and local-mode flag; do not refetch existing comments. In local mode use `read_file`/`glob`/`grep_search`; otherwise use `gh api` via `run_shell_command`.
