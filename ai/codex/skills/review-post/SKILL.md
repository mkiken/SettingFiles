...
```
   ...
   - This multi-line body is authoritative for this skill and overrides the shared `pr_post_mechanics_core.md` Posting section's one-line example (`{priority_emoji} **{Priority}** / **{Category}**: {Description}`). Never collapse the body down to that one-line form or drop the detail text to match it — `{Description}` there stands for this section's full body, not a substitute for it.
...
```bash
posted_comments=$(gh api "repos/{owner}/{repo}/pulls/{pr_number}/comments?per_page=100" --paginate --slurp \
