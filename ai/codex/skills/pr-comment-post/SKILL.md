...
Look at the conversation history for `pr-review` output. For each requested number N, extract:
...
- The AI header (`🤖 **Codex Review**`) appears only once in the review body, not in each inline comment.
