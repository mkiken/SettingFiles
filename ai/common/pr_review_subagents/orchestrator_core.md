...
Fetch each gather target at most once. Preserve the full raw payload for final aggregation, but return only bounded derived results to the parent — the shared `Dynamic Result Output` rule's count-and-summarize threshold applies here. Never repeat a fetch to recover data that the configured large-output path already captured. When the large-output path writes temp files (e.g. a redirected diff or comment dump), write them under the session scratchpad directory, never inside the reviewed repository's working tree — a file left there stays untracked and makes later tooling treat the repository as dirty. If one does end up in the repository, delete it with `trash` (never `rm`) before finishing the review.
