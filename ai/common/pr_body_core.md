...
- If `.github/PULL_REQUEST_TEMPLATE.md` exists in the repository root, use its structure as the base for the PR body and fill in each section with the generated content. If it does not exist, use the default section format below.
- Preserve meaningful information from the existing PR body: manually written TODOs, FIXME notes, free-form notes, incomplete checklists, HTML comments, review requests, and useful background context.
...
- Output **raw markdown format** that can be directly copied to PR body, wrapped in ``` code blocks.
...
3. **Display the machine-generated diff** between existing body and new body:
   ...
   - Run the diff command and paste its output **verbatim** inside a fenced code block with the `diff` language tag, so additions (`+`) render green and deletions (`-`) render red:
     ```bash
     diff -u <tmpdir>/pr_body_old.md <tmpdir>/pr_body_new.md
     ```
   - The code fence must be longer than any backtick run inside the diff output — use at least four backticks (````diff). PR bodies usually contain ``` code blocks, and a three-backtick fence would be closed early by them, breaking the color highlighting. If the output contains a four-backtick run, use five
