...
Before anything else, look at the conversation history for `my:pr-review` output and build an internal numbered index of all findings in this compact format — one line per item:
...
Do not display this full index when `$ARGUMENTS` already specifies item numbers. The posting preview must show only the items that will be posted.
...
If the user confirms, continue with exactly the displayed posting list. If the user does not confirm or wants a different item set, abort without posting and ask them to rerun the command with the desired item numbers.
