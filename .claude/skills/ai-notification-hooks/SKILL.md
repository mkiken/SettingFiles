...
- Notification sound is chosen by event type, not by agent: `ai_notification_sound <event>` (`shell/tmux/ai_notification_sound.sh`) maps `completed`→Hero / `waiting`→Glass / `error`→Basso (unknown→default). The map is the single source of truth, sourced by both the tmux common header and the Herdr `notify-rich` plugin (`notify-on-agent-status.sh`, keyed on `agent_status` done/blocked), so tmux and Herdr play the same sound for the same event across all AIs. Context-alert (Tink) stays separate in `shell/zsh/alias/context-alert.zsh`.
