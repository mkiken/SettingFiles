...
# PRレビューと設定監査で同じサーバーを使う。run_dir内のマニフェストファイル名でどちらかを判別する。
PROFILES = {
    "review": {
    ...
    "audit": {
        "manifest": "audit.json",
        "schema_version": 1,
        "decisions": frozenset({"apply", "dismiss", None}),
    },
}


def detect_profile(run_dir):
    for name, profile in PROFILES.items():
        if (Path(run_dir) / profile["manifest"]).is_file():
            return name
    return None


def expected_item_ids(run_dir, profile_name):
    manifest = PROFILES[profile_name]["manifest"]
    with (run_dir / manifest).open(encoding="utf-8") as source:
        data = json.load(source)
    return {str(item["id"]) for item in data.get("items", [])}


def validate_state(payload, item_ids, profile_name):
    ...
    for item_id, entry in items.items():
        ...
        if decision not in allowed:
            names = " / ".join(sorted(v for v in allowed if v is not None))
            raise ValueError(f"decision must be one of {names} / null")
...
class ReviewServer(ThreadingHTTPServer):
    ...
    def __init__(self, run_dir, idle_timeout, profile_name=None):
        self.run_dir = Path(run_dir).resolve()
        self.report_path = self.run_dir / "report.html"
        self.state_path = self.run_dir / "state.json"
        self.server_info_path = self.run_dir / ".server.json"
        self.profile_name = profile_name or detect_profile(self.run_dir)
        if self.profile_name is None:
            raise ValueError("run_dir contains neither merged.json nor audit.json")
        self.item_ids = expected_item_ids(self.run_dir, self.profile_name)
...
class ReviewHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            validate_state(payload, self.server.item_ids, self.server.profile_name)
...
def serve(run_dir, idle_timeout=43200, open_browser=False, profile_name=None):
    ...
    server = ReviewServer(run_dir, idle_timeout, profile_name)
...
def main(argv=None):
    ...
    profile_name = args.profile or detect_profile(args.run_dir)
    if profile_name is None:
        parser.error("run_dir must contain merged.json or audit.json")
    manifest = PROFILES[profile_name]["manifest"]
    if not (args.run_dir / manifest).is_file() or not (args.run_dir / "report.html").is_file():
        parser.error(f"run_dir must contain {manifest} and report.html")
    serve(args.run_dir, args.idle_timeout, args.open_browser, profile_name)
    return 0
