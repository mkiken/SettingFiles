...
a{color:var(--link)} button{cursor:pointer;border:1px solid var(--border);background:var(--card);color:var(--text);border-radius:6px;padding:5px 10px;font-size:13px}
...
<button id="connect-state">進捗を保存…</button><span id="save-status">未接続</span><span class="save-help">確認・対応状態を state.json に保存します</span>
</div>
<div id="report" class="hide-completed"></div>
<footer><span id="progress"></span><button id="toggle-completed">処理済みを表示</button><span id="params"></span><button id="copy-params">番号をコピー</button></footer>
<script>
...
function decision(id){const group=el("fieldset","decision");group.setAttribute("aria-label","指摘の処理状態");for(const [value,label] of [["none","未選択"],["reviewed","確認済み（対応不要）"],["adopt","対応リストへ追加"]]){const labelEl=el("label","",label);const input=document.createElement("input");input.type="radio";input.name="decision-"+id;input.value=value;input.addEventListener("change",()=>{const s=itemState(id);s.reviewed=value==="reviewed";s.adopt=value==="adopt";setOpen(input.closest(".card"),false);refresh();scheduleSave();});labelEl.prepend(input);group.appendChild(labelEl);}return group;}
...
function refresh(){let reviewed=0;const adopted=[];for(const item of DATA.items){const s=itemState(item.id);if(s.reviewed)reviewed++;if(s.adopt)adopted.push(item.id);const c=document.querySelector(`.card[data-id="${item.id}"]`);if(c){const completed=s.reviewed||s.adopt;c.classList.toggle("completed",completed);c.classList.toggle("adopted",s.adopt);const status=c.querySelector(".decision-status");status.textContent=s.reviewed?"確認済み（対応不要）":s.adopt?"対応リストへ追加済み":"";status.className="decision-status"+(s.reviewed?" badge done":s.adopt?" badge adopted":"");for(const input of c.querySelectorAll(".decision input"))input.checked=input.value===(s.reviewed?"reviewed":s.adopt?"adopt":"none");}}for(const group of document.querySelectorAll(".priority-group")){const cards=group.querySelectorAll(".card");group.classList.toggle("all-completed",cards.length>0&&[...cards].every(c=>c.classList.contains("completed")));}const done=reviewed+adopted.length;const root=document.getElementById("report");root.classList.toggle("hide-completed",!showCompleted);document.getElementById("progress").textContent=`未処理 ${DATA.items.length-done}/${DATA.items.length}（確認済み ${reviewed} / 対応 ${adopted.length}）`;document.getElementById("toggle-completed").textContent=showCompleted?`処理済みを隠す (${done})`:`処理済みを表示 (${done})`;document.getElementById("params").textContent=adopted.length?`対応する: ${adopted.join(",")}`:"対応する: (未選択)";}
async function connectState(){try{fileHandle=await window.showSaveFilePicker({suggestedName:"state.json",types:[{description:"JSON",accept:{"application/json":[".json"]}}]});}catch(e){return;}try{const text=await(await fileHandle.getFile()).text();if(text.trim()){const loaded=JSON.parse(text);if(loaded&&loaded.items)Object.assign(state.items,loaded.items);}}catch(e){}refresh();await save();}
function scheduleSave(){if(!fileHandle)return;clearTimeout(saveTimer);saveTimer=setTimeout(save,300);}async function save(){if(!fileHandle)return;const w=await fileHandle.createWritable();await w.write(JSON.stringify(state,null,1));await w.close();document.getElementById("save-status").textContent="保存済み "+new Date().toLocaleTimeString();}
...
document.getElementById("expand-all").addEventListener("click",()=>document.querySelectorAll(".card").forEach(c=>setOpen(c,true)));document.getElementById("collapse-all").addEventListener("click",()=>document.querySelectorAll(".card").forEach(c=>setOpen(c,false)));document.getElementById("connect-state").addEventListener("click",connectState);document.getElementById("toggle-completed").addEventListener("click",()=>{showCompleted=!showCompleted;refresh();});document.getElementById("copy-params").addEventListener("click",()=>navigator.clipboard.writeText(DATA.items.filter(i=>itemState(i.id).adopt).map(i=>i.id).join(",")));document.querySelectorAll(".theme-picker button").forEach(b=>b.addEventListener("click",()=>setTheme(b.dataset.theme)));let savedTheme="auto";try{savedTheme=localStorage.getItem(THEME_KEY)||"auto";}catch(e){}setTheme(savedTheme);if(!window.showSaveFilePicker){document.getElementById("connect-state").disabled=true;document.getElementById("save-status").textContent="このブラウザは進捗保存に非対応(Chrome系で開いてください)";}build();
</script>
</body>
</html>
"""
