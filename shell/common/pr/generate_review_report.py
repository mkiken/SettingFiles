...
<div id="report"></div>
<footer><span id="progress"></span><div class="filter-picker" role="group" aria-label="表示する指摘"><button data-filter="pending">未処理</button><button data-filter="adopt">対応するリスト</button><button data-filter="reviewed">対応しないリスト</button><button data-filter="all">すべて</button></div><span id="params"></span><button id="copy-run-dir" disabled>実行ディレクトリをコピー</button><button id="save-state" disabled>状態ファイルを保存</button><span id="save-status" role="status" aria-live="polite">未保存</span></footer>
...
const CARRY = {skipped_before:"前回スキップ",should_be_fixed:"前回対応済のはず",fixed_before:"前回修正済み（再指摘）",fix_skipped_before:"前回修正スキップ",fix_rejected_before:"前回修正却下"};
...
function matchesFilter(s){return filterMode==="all"||(filterMode==="pending"&&!s.reviewed&&!s.adopt)||(filterMode==="adopt"&&s.adopt)||(filterMode==="reviewed"&&s.reviewed);}
function refresh(){let reviewed=0;const adopted=[];for(const item of DATA.items){const s=itemState(item.id);if(s.reviewed)reviewed++;if(s.adopt)adopted.push(item.id);const c=document.querySelector(`.card[data-id="${item.id}"]`);if(c){const completed=s.reviewed||s.adopt;c.classList.toggle("completed",completed);c.classList.toggle("adopted",s.adopt);c.classList.toggle("skipped",s.reviewed);c.hidden=!matchesFilter(s);const status=c.querySelector(".decision-status");status.textContent=s.adopt?"🔧 対応する":s.reviewed?"🚫 対応しない":"";status.className="decision-status"+(s.adopt?" badge adopted":s.reviewed?" badge skipped":"");for(const input of c.querySelectorAll(".decision input"))input.checked=input.value==="adopt"?s.adopt:s.reviewed;}}for(const group of document.querySelectorAll(".priority-group"))group.hidden=![...group.querySelectorAll(".card")].some(c=>!c.hidden);const done=reviewed+adopted.length;const counts={pending:DATA.items.length-done,adopt:adopted.length,reviewed,all:DATA.items.length};document.getElementById("progress").textContent=`未処理 ${counts.pending}/${DATA.items.length}（対応しない ${reviewed} / 対応する ${adopted.length}）`;document.getElementById("params").textContent=adopted.length?`対応する: ${adopted.join(",")}`:"対応する: (未選択)";for(const button of document.querySelectorAll("[data-filter]")){button.setAttribute("aria-pressed",String(button.dataset.filter===filterMode));button.textContent=`${{pending:"未処理",adopt:"対応するリスト",reviewed:"対応しないリスト",all:"すべて"}[button.dataset.filter]} (${counts[button.dataset.filter]})`;}document.getElementById("save-state").disabled=!CAN_SAVE_STATE||done!==DATA.items.length;}
