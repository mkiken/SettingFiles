...
async function saveState(){if(!CAN_SAVE_STATE||!allItemsCompleted())return;const status=document.getElementById("save-status");status.textContent="保存中…";try{if(CAN_SERVER_SAVE){const response=await fetch("/api/state",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(state)});if(!response.ok)throw new Error(await response.text());}else{fileHandle=await window.showSaveFilePicker({suggestedName:"state.json",types:[{description:"JSON",accept:{"application/json":[".json"]}}]});const writer=await fileHandle.createWritable();await writer.write(JSON.stringify(state,null,1));await writer.close();}status.textContent="保存済み "+new Date().toLocaleTimeString();}catch(e){if(e&&e.name==="AbortError"){status.textContent="保存を取り消しました";return;}status.textContent="保存に失敗しました。";}}
...
document.getElementById("expand-all").addEventListener("click",()=>document.querySelectorAll(".card").forEach(c=>setOpen(c,true)));document.getElementById("collapse-all").addEventListener("click",()=>document.querySelectorAll(".card").forEach(c=>setOpen(c,false)));document.getElementById("save-state").addEventListener("click",saveState);document.querySelectorAll("[data-filter]").forEach(b=>b.addEventListener("click",()=>{filterMode=b.dataset.filter;refresh();}));document.querySelectorAll(".theme-picker button").forEach(b=>b.addEventListener("click",()=>setTheme(b.dataset.theme)));let savedTheme="auto";try{savedTheme=localStorage.getItem(THEME_KEY)||"auto";}catch(e){}setTheme(savedTheme);if(!CAN_SAVE_STATE)document.getElementById("save-status").textContent="進捗保存に非対応です。レビューコマンドから開き直してください。";build();heartbeat();if(CAN_SERVER_SAVE)window.setInterval(heartbeat,60000);
</script>
</body>
</html>
"""
